...
#3.--------------------Figure 4A Peptides FDR-----------------------------------

#Peptides and false number of each Mix when count <= 3
pepFDR_181 <- FDR_list_181_withoutcarry[[3]]
pepFDR_19 <- FDR_list_19_withoutcarry[[3]]
...
#signif difference
t_test_peptides <- t.test(pepFDR$FDR_181, pepFDR$FDR_19, paired = TRUE) 
...
figure_4a_right <- ggplot(pepFDR_long,aes(x=Mixtures,y=FDR,fill=version))+
...
figure_4a_left <- ggplot(pepFDR_long, aes(x = version, y = FDR, fill = version)) +
...
figure_4a <- figure_4a_right + figure_4a_left + plot_layout(widths = c(3, 1))
figure_4a

#4. -------------------Figure 4B protein FDR-------------------------------------

# 4.1  versionb 1.8.1

# Set working directory for DIANN181

setwd() #again for reading output of diann 1.8.1
...
for (i in 1:12) {
  MixList_181_withoutcarry[[i]] <- MixList_181_withoutcarry[[i]] %>%
...
for (i in 1:12) {
  type1_181[[i]] <- MixList_181_withoutcarry[[i]] %>%
    filter(!grepl("/", Protein.Group))
  
  type2_181[[i]] <- MixList_181_withoutcarry[[i]] %>%
    filter(grepl("/", Protein.Group)) %>%
    filter(grepl(paste0("Mix", i, "_"), Protein.Group)) %>%
    separate_rows(Protein.Group, sep = "/") %>%
    filter(grepl(paste0("Mix", i, "_"), Protein.Group))
  
  type3_181[[i]] <- MixList_181_withoutcarry[[i]] %>%
    filter(grepl("/", Protein.Group)) %>%
    filter(!grepl(paste0("Mix", i, "_"), Protein.Group))
  
  MixList_181_withoutcarry[[i]] <- bind_rows(type1_181[[i]], type2_181[[i]], type3_181[[i]]) %>%
  ...
  Mix_pg_all_181[[i]] <- MixList_181_withoutcarry[[i]] %>%
...
# Set working directory for DIANN19
setwd() #again for reading output of diann 1.9
...
for (i in 1:12) {
  MixList_19_withoutcarry[[i]] <- MixList_19_withoutcarry[[i]] %>%
...
for (i in 1:12) {
  type1_19[[i]] <- MixList_19_withoutcarry[[i]] %>%
    filter(!grepl("/", Protein.Group))
  
  type2_19[[i]] <- MixList_19_withoutcarry[[i]] %>%
    filter(grepl("/", Protein.Group)) %>%
    filter(grepl(paste0("Mix", i, "_"), Protein.Group)) %>%
    separate_rows(Protein.Group, sep = "/") %>%
    filter(grepl(paste0("Mix", i, "_"), Protein.Group))
  
  type3_19[[i]] <- MixList_19_withoutcarry[[i]] %>%
    filter(grepl("/", Protein.Group)) %>%
    filter(!grepl(paste0("Mix", i, "_"), Protein.Group))
  
  MixList_19_withoutcarry[[i]] <- bind_rows(type1_19[[i]], type2_19[[i]], type3_19[[i]]) %>%
    mutate(Protein.Group = str_replace_all(Protein.Group, pattern = "Mix[0-9]+_", replacement = "")) %>%
    distinct(Protein.Group, Sequence, .keep_all = TRUE)
  
  Mix_pg_all_19[[i]] <- MixList_19_withoutcarry[[i]] %>%
    mutate(Name = paste(Protein.Group, Sequence, sep = ",")) %>%
    select(Name, Count)
}

# Prepare TheoreticalPeptideList for DIANN19
TheoreticalPepList_for19 <- list()
for (i in 1:12) {
  TheoreticalPepList_for19[[i]] <- TheoreticalPepList[[i]] %>%
    mutate(Name = paste(Protein.Group, Sequence, sep = ",")) %>%
    select(Name)
}
...
for (i in 1:12) {
  Mix_pg_all_19[[i]] <- Mix_pg_all_19[[i]] %>%
    mutate(Exist = ifelse(Name %in% TheoreticalPepList_for19[[i]]$Name, 1, 0)) %>%
    separate(Name, c("Protein.Group", "Sequence"), sep = ",")
}
...
#5.--------------------Figure 4C True protein_peptides per proteins------------
...
#6.--------------------Figure 4D False protein_peptides per proteins------------
...
#7.--------------------Figure 4E protein identified by one peptide------------
