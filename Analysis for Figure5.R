#-----------------Figure 5  diann 1.8.1 vs 1.9 in Identification----------------

source("Figure4.R")

#1.--------------------------Figure 5A-------------------------------------------

# total number of peptide, read theoretical peptides and filter
setwd() #again for reading Theoretical Peptides
...
#combine
venn_pep <- bind_rows(venn_pep_181, venn_pep_19) %>% 
  mutate(diann181 = ifelse(Sequence %in% venn_pep_181$Sequence, 1, 0),
         diann19 = ifelse(Sequence %in% venn_pep_19$Sequence, 1, 0))
...
#combine
venn_pg <- bind_rows(venn_pg_181, venn_pg_19) %>% 
  mutate(diann181 = ifelse(Protein.Group %in% venn_pg_181$Protein.Group, 1, 0),
         diann19 = ifelse(Protein.Group %in% venn_pg_19$Protein.Group, 1, 0))
...
figure_5 <- figure_5a/figure_5b/figure_5d
